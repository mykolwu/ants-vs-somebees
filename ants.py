...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not place.is_hive:
            bees = place.bees
            if (self.lower_bound <= dist <= self.upper_bound) and bees:
...
class WallAnt(Ant):
    """WallAnt has a large amount of health."""
...
class HungryAnt(Ant):
    """HungryAnt will take three turns to chew a Bee in its place.
    While still chewing, the HungryAnt can't eat another Bee.
    """
    name = 'Hungry'
    food_cost = 4
    implemented = True
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is a ThrowerAnt which is waterproof."""
    ...
    def throw_at(self, target):
        ...
        if target:
            ...
            def slow_action(gamestate):
                if target.remaining_slow_turns:
